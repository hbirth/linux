					  ...
					  pgoff_t end, bool may_be_dirty)
{
	 ...
	 * fuse_launder_folio() is handed one folio at a time and sends a
	 * FUSE_WRITE for each, where a writeback pass batches the same bytes
	 * up to fc->max_write; what it sends the drop then only waits for.
	 */
			     ...
			     loff_t offset, loff_t len)
{
	...
	if (offset >= 0) {
		 ...
		 *
		 * A fenced writer may be waiting for a FUSE_WRITE reply, so
		 * this waits on the server the same way the flush below does.
		 *
		 ...
		 *    read returns a folio the remote modify has superseded.
		 *    Nothing is fenced out for it.  A read racing the drop
		 *    either misses and refetches or returns data that was
		 *    current when it was copied.  A write racing it is caught
		 *    on the way out instead: this revoke marks the range rather
		 *    than forgetting it, and writeback holds the range again
		 *    before sending anything it finds marked that way.
		 *
		...
		if (tracked) {
			 ...
			 * launders when the range may be dirty and so waits
			 * for these same replies.  One explicit wait, before
			 * the revoke, and the drop then finds nothing under
			...
			if (fuse_inode_force_dio(inode)) {
				...
				fuse_invalidate_mapping_range(inode, 0, -1, dirty);
			} else if (has_pages) {
