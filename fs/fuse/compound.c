		      ...
		      struct fuse_args *args)
{
	...
	printk("fuse_compound_add: op[%u] opcode=%u, expected_out_size=%zu, total_expected=%zu\n",
	       compound->compound_header.count, args->opcode, expected_out_size,
	       compound->total_expected_out_size);
...
static void *fuse_copy_response_data(struct fuse_args *args, char *response_data)
{
	...
	printk("fuse_copy_response_data: args=%p, out_numargs=%d, response_data=%p\n",
	       args, args->out_numargs, response_data);

	for (arg_idx = 0; arg_idx < args->out_numargs; arg_idx++) {
		struct fuse_arg current_arg = args->out_args[arg_idx];
		printk("fuse_copy_response_data: arg[%d] size=%zu, value=%p, out_pages=%d\n",
		       arg_idx, current_arg.size, current_arg.value, args->out_pages);
		...
		if (arg_idx == args->out_numargs - 1 && args->out_pages) {
			 ...
			 */
			printk("fuse_copy_response_data: skipping out_pages arg[%d]\n", arg_idx);
			return response_data;
		} else {
			size_t arg_size = current_arg.size;
			if (current_arg.value && arg_size > 0) {
				printk("fuse_copy_response_data: copying %zu bytes from offset %zu to %p\n",
				       arg_size, copied, current_arg.value);
				memcpy(current_arg.value,
				       (char *)response_data + copied,
				       arg_size);
				copied += arg_size;
			} else {
				printk("fuse_copy_response_data: skipping arg[%d] (value=%p, size=%zu)\n",
				       arg_idx, current_arg.value, arg_size);
			}
		}
	}
	printk("fuse_copy_response_data: total copied=%zu bytes, returning %p\n",
	       copied, (char*)response_data + copied);
				...
				uint32_t count, void *response, size_t response_size)
{
	...
	printk("fuse_compound_parse_resp: response=%p, response_size=%zu, count=%u, compound_header.count=%u\n",
	       response, response_size, count, compound->compound_header.count);

	/* Basic validation */
	if (!response || response_size < sizeof(struct fuse_out_header)) {
		printk("fuse_compound_parse_resp: invalid response buffer\n");
		return -EIO;
	}
	...
	for (i = 0;
		...
		printk("fuse_compound_parse_resp: op[%u] op_hdr=%p, len=%u, error=%d, unique=%llu\n",
		       i, op_hdr, op_hdr->len, op_hdr->error, op_hdr->unique);

		/* Validate header length */
		if (op_hdr->len < sizeof(struct fuse_out_header)) {
			printk("fuse_compound_parse_resp: op[%u] invalid header length %u\n", i, op_hdr->len);
			return -EIO;
		}

		/* Check if the entire operation response fits in the buffer */
		if ((char *)op_out_data + op_hdr->len > (char *)response_end) {
			printk("fuse_compound_parse_resp: op[%u] response extends beyond buffer (len=%u)\n", i, op_hdr->len);
			return -EIO;
		}

		if (op_hdr->error != 0) {
			compound->op_errors[i] = op_hdr->error;
			printk("fuse_compound_parse_resp: op[%u] error=%d\n", i, op_hdr->error);
			if ((compound->compound_header.flags & FUSE_COMPOUND_CONTINUE) == 0) {
				/* if we are not supposed to continue
		...
		if (args && op_hdr->len > sizeof(struct fuse_out_header)) {
			printk("fuse_compound_parse_resp: op[%u] copying response data, data_len=%u\n",
			       i, op_hdr->len - sizeof(struct fuse_out_header));
			...
			printk("fuse_compound_parse_resp: op[%u] no response data, advancing by %u bytes\n",
			       i, op_hdr->len);
			op_out_data = (char *)op_out_data + op_hdr->len;
		}
		printk("fuse_compound_parse_resp: op[%u] next op_out_data=%p\n", i, op_out_data);
	}
...
/* kernel does not know this error code
 * but we need it for combinations of operations
 * in a compound that are not supported by the fuse server.
 * If the fuse server would send ENOSYS instead we would disable
 * compounds, which is not correct in this case.
 */
#define ENOTSUP 95
				...
				struct fuse_compound_req *compound)
{
	...
	printk("fuse_compound_send: total_expected_out_size=%zu, count=%u, header_size=%zu\n",
	       compound->total_expected_out_size, compound->compound_header.count,
	       sizeof(struct fuse_out_header));
	printk("fuse_compound_send: allocating buffer of size %zu\n", total_buffer_size);
	...
	printk("fuse_compound_send: result size %zu\n",
		compound->compound_header.result_size);

	ret = fuse_compound_idmap_request(idmap, compound->fm, &args);
	printk("fuse_compound_send: fuse_compound_request returned %zd\n", ret);
	if (ret == -ENOSYS) {
	...
	if (ret == -ENOTSUP) {
		/* this operation combination is not supported */
		ret = -ENOSYS;
		goto out;
	}
	...
	printk("fuse_compound_send: expected_response_size=%zu, actual_response_size=%zu\n",
	       expected_response_size, actual_response_size);
	printk("fuse_compound_send: result_header.count=%u, result_header.flags=%u\n",
	       compound->result_header.count, compound->result_header.flags);

	/* Validate response size */
	if (actual_response_size < sizeof(struct fuse_compound_out)) {
		printk("fuse_compound_send: response too small: %zu < %zu\n",
