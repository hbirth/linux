...
static int fuse_compound_open_getattr(struct fuse_mount *fm, u64 nodeid,
				int openflags, int openopcode,
				struct fuse_file *ff,
				struct fuse_open_out *outopenargp,
				struct fuse_attr_out *outattrp)
{
	...
	compound = fuse_compound_alloc(fm, FUSE_COMPOUND_ORDERED);
	if (IS_ERR(compound))
		return PTR_ERR(compound);

	/* Add OPEN */
	open_in.flags = openflags & ~(O_CREAT | O_EXCL | O_NOCTTY);
	if (!fm->fc->atomic_o_trunc)
	...
	open_args.out_args[0].value = outopenargp;
	...
	ff->fh = outopenargp->fh;
	ff->open_flags = outopenargp->open_flags;
